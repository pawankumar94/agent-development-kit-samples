...
def create_weather_agent() -> LlmAgent:
    """
    Creates and configures the weather assistant agent.
    
    Returns:
        Configured LlmAgent instance ready for use.
    """
    ...
    agent = LlmAgent(
        ...
        model=DEFAULT_MODEL,
        tools=[get_weather, calculate_math, get_time_info],
        instruction="""You are a helpful weather assistant that can:
        1. Check weather for any city using the get_weather tool
        2. Perform mathematical calculations using the calculate_math tool  
        3. Get current time information using the get_time_info tool
        
        When users ask about weather, use the get_weather tool with the city name.
        When users ask for calculations, use the calculate_math tool with their expression.
        When users ask about time, use the get_time_info tool with the timezone if provided.
        
        Always be friendly, helpful, and provide clear responses based on the tool results.
        If a tool returns an error, explain the issue and suggest alternatives.""",
        description="A helpful assistant that provides weather information, performs calculations, and gives time information"
    )
...
def demo_weather_assistant():
    ...
    session_manager = ADKSessionManager(app_name="weather_assistant_demo")
